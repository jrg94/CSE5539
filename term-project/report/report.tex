...
data > moov > udta > meta > ilst > gnre. As an added wrinkle, most of the meta
data leverages numerical codes. For instance, genres are mapped to numbers, so
there's a separate lookup table for genre strings. For the sake of simplicity,
I only chose to track all the fields that you can see in the JSON above.
